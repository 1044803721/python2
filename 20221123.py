...
import numpy as np
from CBP.compact_bilinear_pooling import compact_bilinear_pooling_layer
class MSRLSTM:
    ...
    def cnn_layer(self, gyr_x_cnn, gyr_y_cnn, gyr_z_cnn, lacc_x_cnn, lacc_y_cnn, lacc_z_cnn, mag_x_cnn, mag_y_cnn,
        ...
        print(lacc_x_cnn.shape,lacc_y_cnn.shape)
        方法1
        with tf.Session() as sess:
            lacc_x_cnn=lacc_x_cnn.eval(session=sess)
            print(lacc_x_cnn.type)
        方法2
        sess = tf.Session()
        lacc_x_cnn=lacc_x_cnn.numpy().tolist()
        print(lacc_x_cnn.type)
        lacc_x_cnn = sess.run(lacc_x_cnn,feed_dict={lacc_x_cnn})'''


        print(lacc_x_cnn.shape, lacc_y_cnn.shape)
        concat_lacc = compact_bilinear_pooling_layer(lacc_x_cnn,lacc_y_cnn)
        print(lacc_x_cnn.shape, lacc_y_cnn.shape)
        concat_gyr = mcb([gyr_x_cnn], [gyr_y_cnn])
        concat_mag = mcb([mag_x_cnn], [mag_y_cnn])
